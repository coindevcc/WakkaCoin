...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // WakkaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "WakkaCoin Signed Message:\n";
...
static const int64 nTargetTimespan = 60 * 60 * 24 * 2;  // WakkaCoin: every block
static const int64 nTargetSpacing = 15;  // WakkaCoin: 30 sec
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal WakkaCoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "WakkaCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "April 21st, 2014 9AM EST";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // WakkaCoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
